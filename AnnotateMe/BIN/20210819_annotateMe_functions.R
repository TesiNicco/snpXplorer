...
GTEx_me_generic <- function(mapping, gtex, mapping.ens){
  ...
  df_in_hg38$chr = str_split_fixed(df_in_hg38$chr, "chr", 2)[, 2]
...
## grep all eqtl (from all tissues) from my permutation dataset
GTEx_me_generic_allTissues <- function(mapping, mapping.ens, interesting_tissues){
  ...
  for (chrom in unique(mapping$chr)){
    ...
    # match in gtex data
    patterns = paste0(target$code_gtex, collapse = "|")
    matches = ref[grep(pattern = patterns, x = ref$V1),]
    # save matches -- we'll merge things afterwards
    all_matches[[length(all_matches)+1]] = matches
  }
  all_matches = rbindlist(all_matches)
  
  #interesting_tissues = unlist(strsplit(interesting_tissues, ","))
  # now we need to assign to each snp -- loop on each of the snps
  mapping$eqtl <- NA
  mapping$eqtl_tissue <- NA
  for (snp in 1:nrow(mapping)){
    try_grep = all_matches[grep(pattern = mapping$code_gtex[snp], x = all_matches$V1),]
    if (nrow(try_grep) >= 1){
      # split all tissues
      all_tissues = unlist(strsplit(try_grep$V2, ";"))
      for (t in all_tissues){
        # identify tissue name and gene
        tissue_name = paste0(unlist(strsplit(t, "_"))[1:(grep("ENS", unlist(strsplit(t, "_")))-1)], collapse = "_")
        gene_name = mapping.ens$gene[which(mapping.ens$ensemble == str_split_fixed(unlist(strsplit(t, "_"))[grep("ENS", unlist(strsplit(t, "_")))], "\\.", 2)[, 1])]
        if (tissue_name %in% interesting_tissues){
          if (length(gene_name) != 0){
            # and then save these things
            if (is.na(mapping$eqtl[snp])){ 
              mapping$eqtl[snp] = gene_name
              mapping$eqtl_tissue[snp] = tissue_name 
            } else {
              mapping$eqtl[snp] = paste(mapping$eqtl[snp], gene_name, sep = ",")
              mapping$eqtl_tissue[snp] = paste(mapping$eqtl_tissue[snp], tissue_name, sep = ",")
            }
          }
        }
      }
...
# function to do the annotation, per variant
# i is the index of the snp_list vector of snps -- genome_version is either hg19 or hg38 -- input_type is locus for chr:pos -- tab for chr pos 
function_caddAnnot_v6 <- function(i, snp_list, genome_version, input_type){
  ...
  # run cadd 
  system(paste0("curl -i https://cadd.gs.washington.edu/api/v1.0/", genV, "/", input, " | tail -n +8 > tmp.json"))
  
  # read json file
  json_data <- fromJSON(file = "tmp.json")  
...
## function to read and store cadd files -- API cadd v2
readcadd_v2 <- function(data, MAIN, ld_info, ref_genome){
  # run function in mp using cadd api depending on ref genome
  # annotate the snp_list
  snp_list = paste(data$chr, data$pos, sep=":")
  all_res = rbindlist(mclapply(X = 1:length(snp_list), FUN = function_caddAnnot_v6, snp_list = snp_list, genome_version = "hg19", input_type = "locus", mc.cores = 1))
...
AnnotateMe <- function(data, genes, gtex, mapping.ens, ftype, MAIN, ld_info, cadd){
  ...
  out.gtex <- GTEx_me_generic_allTissues(mapping = out.annot, mapping.ens = mapping.ens, interesting_tissues = interesting_tissues)
...
readSNPs <- function(fname, ftype, MAIN, ref_version){
  ...
  if (ftype == 1){
    ...
    if (ref_version == "GRCh38"){
      ...
      gr <- makeGRangesFromDataFrame(df)
      # set chain file
      chain <- import.chain(paste(MAIN, "BIN/hg38ToHg19.over.chain", sep=""))
      # change coordinates
      gr_hg38 <- liftOver(gr, chain)
      # back to dataframe and clean it
      df_hg38 <- as.data.frame(gr_hg38)
      ...
      final$chr_n = str_split_fixed(final$chr, "chr", 2)[, 2]
    ...
    info = as.data.frame(str_split_fixed(info, "\t", 7))
...
revigoPlot <- function(one.data){
  ...
  ## colors
  colz <- viridis(n = nrow(one.data), option = "plasma")
  ...
  sb = head(one.data, n)
  addTextLabels(sb$PlotX, sb$PlotY, sb$upd_name, col.label="black")
  ...
  gradient.rect(xleft = mx.x-(step*2), ybottom = mx.y-(step*9), xright = mx.x+(step*2), ytop = mx.y-(step*2), col=rev(colz), nslices = length(colz), gradient = "horizontal")
