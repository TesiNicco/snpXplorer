...
## If memory is enough, let's start
if (START == TRUE){
  # LIBRARIES
  ...
  data <- try(readSNPs(fname, ftype, MAIN, ref_version), silent = T)
  ...
  ## check whether input was correct or not, in case it is not, send an email as notification
  if (length(data) == 1){
    ...
    if (length(unique(geneList)) >1){
      ...
      if (length(grep("GO", dbs)) >0){
        ...
        # release some memory before going on
        gc()
        ...
        semsim_results = try(expr = alternative_revigo_results(MAIN, random_num, go_data), silent = T)
        # check for errors
      ...
      system(cmd_compress)
      #cmd_mail <- paste("sendEmail -f snpXplorer@gmail.com -t ", username, " -u 'AnnotateMe results' -m 'Dear user, \n thanks so much for using snpXplorer and AnnotateMe. \n We hope you find the tool useful. \n AnnotateMe team.' -a 'AnnotateMe_results_", random_num, ".tar.gz' -cc n.tesi@amsterdamumc.nl -s smtp.gmail.com:25 -xu snpXplorer@gmail.com -xp snpXplorer22101991!", sep="")
      cmd_mail <- paste0("sendEmail -f snpxplorer@gmail.com -t ", username, " -u 'snpXplorer results' -m 'Dear user, \n thanks so much for using snpXplorer and its annotation pipeline. \n We hope you find the tool useful. \n snpXplorer team.' -a 'AnnotateMe_results_", random_num, ".tar.gz' -cc n.tesi@amsterdamumc.nl snpxplorer@gmail.com -S /usr/sbin/sendmail")
      system(cmd_mail)
      # finally also remove input annotateME list of snps
      cmd = paste("rm ", fname, sep="")
      system(cmd)
