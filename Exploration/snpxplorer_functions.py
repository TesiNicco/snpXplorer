...
def check_runID(run_id):
    ...
    if run_id == '':
        ...
        if os.path.exists('/Annotation/RUNS/snpXplorer_annotation_%s.zip' %(run_id)):
            messageError = False
...
def run_annotation(my_list, inpType, refGeno, analType, gsea_source, qtl_tissues, email):
    ...
    fpath_server = '/Annotation/RUNS/snpXplorer_input_%s.txt' %(random_number)
    ...
    command = f"python /Annotation/BIN/standalone_annotation_optimized.py -q {fpath_server} -t {analysis_type} -o /Annotation/RUNS/ -r {random_number} -b {refGeno} -ts {gtex_tissues} -gs {analysis_mode} -e {email} "
    print(command)
    return command
