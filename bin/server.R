...
function.plot <- function(snp.info, y.lim, type, plt.type, windows.number, smooth.par, 
                          int.locus, gwas, colorPoint, ld, input, genV, inpStrVar, pop_interest_ld){
...
plotEqtl_table <- function(snps_in_interval, eQTL, mapping.ensembl, GENOME){
  # first exclude duplicates based on position
  snps_in_interval <- snps_in_interval[!duplicated(snps_in_interval$pos),]
  snps_in_interval$chrom <- paste("chr", snps_in_interval$chr, sep="")
  # then take eqtl in interval
  if (GENOME == "GRCh37 (hg19)"){
    eqtl_in_interval <- eQTL[which((eQTL$chr %in% snps_in_interval$chrom) & (eQTL$pos_hg37 %in% snps_in_interval$pos)),]
    eqtl_in_interval$Locus_hg37 <- paste(eqtl_in_interval$chr, eqtl_in_interval$pos_hg37, sep=":")
  } else {
    df <- data.frame(chr=paste("chr", snps_in_interval$chr, sep=""), start=snps_in_interval$pos, end=snps_in_interval$pos+1)
    #print(head(df))
    # change to GR class object
    gr <- makeGRangesFromDataFrame(df)
    # set chain file
    ...
    # back to dataframe and clean it
    df_hg38 <- as.data.frame(gr_hg38)
    sb <- data.frame(chr=rep(snps_in_interval$chr[1], nrow(df_hg38)), pos=df_hg38$start)
    colnames(sb) <- c("chr", "pos")
    #print(head(sb))
    eqtl_in_interval <- eQTL[which((eQTL$chr %in% sb$chrom) & (eQTL$pos_hg38 %in% sb$pos)),]    
    eqtl_in_interval$Locus_hg38 <- paste(eqtl_in_interval$chr, eqtl_in_interval$pos_hg38, sep=":")
    #print(head(eqtl_in_interval))
  }
  ...
  if (nrow(eqtl_in_interval) >0){
    ...
    if (GENOME == "GRCh38 (hg38)"){
      eqtl_in_interval <- eqtl_in_interval[, c("Locus_hg38", "a1", "a2", "maf", "slope", "log_adjP", "Gene name")]
    } else {
      eqtl_in_interval <- eqtl_in_interval[, c("Locus_hg37", "a1", "a2", "maf", "slope", "log_adjP", "Gene name")]
    }
    # order by p and return top 15 entries
    colnames(eqtl_in_interval) <- c("Locus", "A1", "A2", "MAF", "Effect", "P", "Gene")
    ...
    eqtl_in_interval <- data.frame(Locus="No", "A1/A2"="eQTLs", MAF="in", Effect="displayed", "P"="region", Gene="-")
  }
  ...
  function(input, output, session) {
    ...
    plt <- function(input, output, session){  
      ...
      if (input$sel == "Locus, Gene, RsID"){
        ...
        if (length(all_gwas) < 2){
          ...
          if (plot_error == TRUE){
            ...
            function.plot(snp.info = res, y.lim = input$y, type = input$sel, plt.type = input$ploType, 
                        ...
                        gwas = gwas, col = input$col, ld = input$Linkage, input = input, genV = input$genV, inpStrVar = input$strVar_inp, pop_interest_ld = pop_interest_ld)
          }
          ...
          if (path_f == "None"){
            ...
            if (plot_error == TRUE){
              ...
              function.multiPlot(snp.info = list_for_results, list_for_loop, y.lim = input$y, type = input$sel, 
                                 ...
                                 int.locus = target, col_list, input$genV, inpStrVar = input$strVar_inp)
            }
            
            # This section is the one with respect to the loaded file -- to be completed  
          } else {
            # loop over the gwas to plot
            # define some outputs -- the first is the names of the gwases
            list_for_loop <- all_gwas
        ...
        all_gwas <- c(input$gwas, input$gwas_from_file)
        if (length(all_gwas) < 2){
          # manage different manual scroll input
          snp.info <- function.InputManualScroll(dat = dat, window = input$x, input.scroll = input$manual.pos, input.chrom = input$manual.chrom, gwas)
          ...
          if (plot_error == FALSE){
            function.plot(snp.info = snp.info, y.lim = input$y, type = input$sel, plt.type = input$ploType, 
                        ...
                        gwas = gwas, col = input$col, ld=input$Linkage, genV = input$genV, inpStrVar = input$strVar_inp, pop_interest_ld = pop_interest_ld)
          } else {
            par(mar=c(4, 8, 10, 6))
            plot(0, 0, xlim=c(0, 55), ylim=c(0, 25), bty="n", xaxt='none', yaxt='none', xlab="", ylab="",
    ...
    output$annot <- renderPlot({
      ...
      if (input$email != "Type your email address..."){
             ...
             annotateMe.cmd <- paste("Rscript /Users/home/Desktop/SNPbrowser/gitHub_version/AnnotateMe/BIN/20200430_AnnotateMe_server.R annotateMe_input_", random_num, ".txt ", ftype, " ", username, " ", analysis_mode,  sep="")
    ...
    output$eQTL <- renderTable({
      ...
      eqtl_in_interval <- plotEqtl_table(snps_in_interval, eQTL, mapping.ensembl, input$genV)
    ...
    # this should be the cross-reference to LD-hub (always naice to put it)
    ...
    output$download_eQTL <- downloadHandler(
      ...
      content = function(file) {
        ...
        # look for eqtl with function
        eqtl_in_interval <- plotEqtl_table(snps_in_interval, eQTL, mapping.ensembl, input$genV)
        write.table(eqtl_in_interval, file, row.names = FALSE, quote=F, sep=" ")
      }
    )
    ...
    output$downloadPlot <- downloadHandler(
      ...
      content = function(file) {
        ...
        png(file = file, height = 20, width = 13, res=300, units="in")
