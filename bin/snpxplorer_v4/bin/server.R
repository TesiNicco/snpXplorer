  ...
  ## PLOTLY FUNCTION FOR PLOT 1 (SNPS, GENES AND SVS)
    output$plot <- renderPlotly({
      ...
      ## DEFINE THE REACTIVE VARIABLES
        ...
        tissues_interest = input$tissues; ld_populations(ld_pop)
      ...
      ## FIND EQTL
        if (plotError == FALSE) { future_eqtl = future({ extractEqtls(region_of_interest, reference_genome, MAIN_PATH, tissues_interest, mapping_ensembl = mapping.ensembl) }) }
      ...
      ## FIND RSID FOR ANNOTATION
        if ((plotError == FALSE)){ future_rsid = future({ findRsID(region_of_interest = region_of_interest, reference_genome = reference_genome, span = 100000, MAIN_PATH, mode = "new") }) }
